...
class CardControllerWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < images!.length; i++) {
      var cardWidth = max(cardMaxWidth - 60 * (currentPostion! - i).abs(), 0.0) + 50;
    ...
    // Last
    var cardWidth = max(cardMaxWidth - 60 * (currentPostion! - images!.length).abs(), 0.0) + 50;
  ...
  double getTop(double cardHeight, double viewHeight, int i) {
    ...
    if (diffAbs > 0.0 && diffAbs <= 1.0) {
      if (diff >= 0) {
        return basePosition - (cardMaxHeight * (6.5 / 9)) * diffAbs;
      } else {
        return basePosition + (cardMaxHeight * (6.5 / 9)) * diffAbs;
      }
      ...
      if (diff >= 0) {
        return basePosition -
            (cardMaxHeight * (6.5 / 9)) -
            cardMaxHeight * (2.5 / 9) * (diffAbs - diffAbs.floor()).abs();
      } else {
        return basePosition +
            (cardMaxHeight * (6.5 / 9)) +
            cardMaxHeight * (2.5 / 9) * (diffAbs - diffAbs.floor()).abs();
      ...
      if (diff >= 0) {
        return basePosition - cardMaxHeight * (8.5 / 9);
      } else {
        return basePosition + cardMaxHeight * (8.5 / 9);
      }
    }
