...
class CardControllerWidget extends StatelessWidget {
  ...
  double getCardHeight(int index) {
    ...
    if (diff >= 0.0 && diff < 1.0) {
      ...
      return (cardMaxHeight - cardMaxHeight * (6 / 10) * ((diff - diff.floor())));
    } else if (diff >= 1.0 && diff < 2.0) {
      print('cardPager : ${index} - 1');
      return cardMaxHeight - cardMaxHeight * (6 / 10) - 10 * ((diff - diff.floor()));
    } else {
      print('cardPager : ${index} - 2');
      final height = cardMaxHeight - cardMaxHeight * (6 / 10) - 10 - 5 * ((diff - diff.floor()));
